...
export async function GET(request: Request) {
    ...
    try {
        ...
        const parsed = JSON.parse(decrypted) as { userId: string; ts: number; service?: string };
    ...
    try {
        await IntegrationService.connectIntegration(userId, service, credentials, accountInfo);
    } catch (error) {
        console.error("[google/callback] Failed to save integration:", error);
        return NextResponse.redirect(`${DASHBOARD_URL}?error=save_failed`);
    }

    return NextResponse.redirect(`${DASHBOARD_URL}?connected=${service}`);
}
