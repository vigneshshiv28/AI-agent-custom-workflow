...
export const Navbar = () => {
  ...
  return (
  <nav className="flex items-center justify-between py-3 px-6 bg-black/95 backdrop-blur-md shadow-[0_4px_12px_rgba(0,0,0,0.25)] rounded-xl">
        ...
        {navLinks.map((link) => {
          ...
          return (
            <Link
              key={link.href}
              href={link.href}
              className={`inline-flex items-center justify-center px-4 py-1 rounded-full text-sm font-medium transition-all duration-300 ${
                active
