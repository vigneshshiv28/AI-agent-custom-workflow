...
import { ArrowLeft, Save, Plus, Play, ChevronUp } from 'lucide-react';
...
/*
  const addNode = useCallback(() => {
    ...
    const newNode: Node = {
      ...
      type: 'custom',
      position: {
        x: lastNode ? lastNode.position.x + 400 : 50,
        y: lastNode ? lastNode.position.y : 250,
      },
      data: {
        label: 'New Action',
        type: 'Action',
  ...
  const handleTest = useCallback(() => {
    ...
    store.nodes.forEach(n => store.updateNodeData(n.id, { ...n.data, runState: 'idle' }));
      ...
      if (activeBranches === 0) {
        setTimeout(() => {
          useWorkflowStore.getState().nodes.forEach(n =>
            useWorkflowStore.getState().updateNodeData(n.id, { ...n.data, runState: 'idle' })
          );
          setIsTesting(false);
    ...
    const runNode = (nodeId: string) => {
      ...
      setTimeout(() => {
        const s2 = useWorkflowStore.getState();
        ...
        if (isError) {
          ...
          setTimeout(() => edgesToRun.forEach(e => runNode(e.target)), 1000);
        }
      }, 1500);
    };
    ...
    <div className="fixed inset-0 z-50 bg-background flex flex-col font-sans">
      ...
      <header className="h-20 border-b border-border bg-card flex items-center justify-between px-8 z-10">
        ...
        <div className="flex items-center gap-4">
          <Button variant="outline" size="md" onClick={addNode} className="border-border hover:border-primary/50">
          ...
          <Button size="md" className="bg-primary text-primary-foreground hover:bg-primary/90 px-8">
            <Save className="w-4 h-4 mr-2" /> Create Workflow
          </Button>
        </div>
      </header>
